...
│   ├── services/                 # Classes de regra de negócio + acesso direto ao banco
│   │   └── UserService.js
│   ├── controllers/              # Classes Controller (orquestração HTTP)
│   │   └── UserController.js
│   ├── middlewares/
...
**M (Model)** → *não existe model separado*
**Service (classe)** → acessa diretamente o banco
**C (Controller classe)** → orquestra HTTP
**View** → qualquer front-end que consome JSON

🟢 **Sem Repositories**
🟢 **Sem Models**
🟢 **Service é sempre uma Classe** (singleton)
🟢 **Controller é sempre uma Classe** com métodos da rota
...
| **Variáveis**      | camelCase                       |
| **NÃO usar**       | models, repositories, callbacks |
...
5. NUNCA expor stack em produção
6. SEMPRE validar dados
7. JSON sempre `{ success: boolean }`
8. Rotas ficam no index.js
9. Service é classe + singleton
10. Controllers usam métodos de classe

---

# Checklist de Implementação

* [ ] Service: classe + SQL + validações
