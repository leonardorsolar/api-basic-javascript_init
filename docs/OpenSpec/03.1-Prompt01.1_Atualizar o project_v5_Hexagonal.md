...
3. Atualize a estrutura de pastas para o padrão **Arquitetura Hexagonal (Ports & Adapters)** conforme abaixo.

Mantenha todas as informações existentes e altere apenas o necessário.

---

# 🗂️ Estrutura de Diretórios — Arquitetura Hexagonal (Ports & Adapters)
```bash
src/
├── core/                          # Núcleo da aplicação (centro do hexágono)
│   └── domain/
...
│       ├── input/                # Portas de entrada (Primary Ports)
│       │   ├── CreateUserPort.js
│       │   ├── ListUsersPort.js
│       │   ├── GetUserByIdPort.js
│       │   ├── UpdateUserPort.js
│       │   └── DeleteUserPort.js
│       └── output/               # Portas de saída (Secondary Ports)
│           ├── UserRepositoryPort.js
│           └── EmailServicePort.js
│
├── adapters/                      # Implementações concretas
│   ├── primary/                  # Adapters de entrada (Driving)
│   │   └── http/
│   │       ├── controllers/
│   │       │   └── UserController.js
│   │       ├── routes/
...
│   │           └── UpdateUserDTO.js
│   │
│   └── secondary/                # Adapters de saída (Driven)
│       ├── repositories/
...
│   └── constants/
│
└── index.js                       # Entry point e DI setup
```

---

# 📐 Convenções de Nomenclatura OBRIGATÓRIAS (Hexagonal)
...
## 1. Core/Domain Layer (Centro do Hexágono)
...
/**
 * Port para listagem de usuários
 */
...
/**
 * Port para buscar usuário por ID
 */
...
* Implementadas pelos adapters secundários

**Exemplo:**
```javascript
// src/application/ports/output/UserRepositoryPort.js
/**
 * Port de repositório de usuários
 */
...
* Dependem apenas de Output Ports (abstrações)
...
export default class CreateUserUseCase extends CreateUserPort {
    ...
    async execute(input) {
        ...
        // Verificar duplicidade (regra de negócio)
...
* Tratam dados de entrada/saída (serialização)
...
## 4. Secondary Adapters (Driven/Saída)
...
// Instanciar adapters secundários (implementações de Output Ports)
const userRepository = new SqliteUserRepository()
const emailService = new NodemailerEmailService()

// Instanciar use cases (injetando dependências)
...
# 🔀 Fluxo de Dependências na Arquitetura Hexagonal
```
┌─────────────────────────────────────────────────────────────┐
│                    PRIMARY ADAPTERS                          │
│                   (HTTP, CLI, GraphQL)                       │
...
**Princípio chave:** As dependências sempre apontam para dentro (em direção ao core/domain)
...
{
    "success": true,
    "data": { "id": 1, "name": "João", "email": "joao@example.com", "age": 25 }
}
```
...
# ✅ Checklist da Arquitetura Hexagonal

- [ ] **Core isolado** - Domínio sem dependências externas
- [ ] **Ports definidos** - Input e Output Ports como interfaces
- [ ] **Use Cases puros** - Dependem apenas de abstrações (Ports)
- [ ] **Primary Adapters** - Controllers/HTTP dependem de Input Ports
- [ ] **Secondary Adapters** - Repositórios implementam Output Ports
- [ ] **Injeção de Dependência** - DI manual ou com container
- [ ] **Fluxo correto** - Dependências apontam para o core
- [ ] **DTOs** - Conversão de dados na camada de adapters
- [ ] **Value Objects** - Objetos imutáveis com validação
- [ ] **Domain Exceptions** - Exceções específicas do domínio
- [ ] **Sem vazamento** - Frameworks não vazam para o core
- [ ] **Testabilidade** - Use Cases testáveis com mocks de Ports
...
2. **Separation of Concerns**
   - Core/Domain: Regras de negócio puras
   - Application: Orquestração (Use Cases)
   - Adapters: Detalhes técnicos

3. **Testability**
   - Core e Use Cases são facilmente testáveis
   - Mocks implementam Output Ports para testes

4. **Technology Agnostic Core**
   - Core não conhece Express, SQLite, etc.
   - Fácil trocar tecnologias (ex: SQLite → PostgreSQL)

---

# 🧪 Exemplo de Teste (Use Case)
```javascript
...
✅ **Testabilidade Superior** - Core e Use Cases facilmente testáveis sem infraestrutura
✅ **Independência de Framework** - Fácil trocar Express por Fastify, etc.
✅ **Independência de Banco** - Fácil migrar SQLite → PostgreSQL → MongoDB
✅ **Manutenibilidade** - Separação clara de responsabilidades
✅ **Flexibilidade** - Múltiplos adapters (HTTP, CLI, GraphQL) compartilham mesmos Use Cases
✅ **Regras de Negócio Protegidas** - Core isolado de detalhes técnicos
